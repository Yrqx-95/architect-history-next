-- Graduation community civic batch 001: reviewed canonical entities and relations.
-- Generated by scripts/prepare-graduation-library-batch-002.mjs (GRADUATION_REVIEW_BATCH=community-civic-001).
-- Scope: 1 new architects, 1 buildings, 1 images,
-- 1 profiles, 3 approved function assignments.
...
BEGIN
  IF to_regclass('public.graduation_case_profiles') IS NULL
    ...
    RAISE EXCEPTION 'Graduation community civic batch 001 prerequisites are missing';
  END IF;
END $$;
...
INSERT INTO architect_seed VALUES
  ('24032837-3d52-5173-8180-dbe970eec9b8'::uuid, 'kazumi-adachi-kiyoshi-sawano-hideo-matsunaga-isamu-noguchi', 'Kazumi Adachi + Kiyoshi Sawano + Hideo Matsunaga + Isamu Noguchi', 'Kazumi Adachi + Kiyoshi Sawano + Hideo Matsunaga + Isamu Noguchi', 'カズミ・アダチ + キヨシ・サワノ + ヒデオ・マツナガ + イサム・ノグチ', 'https://jaccc.org/about/', true);
...
INSERT INTO building_seed VALUES
  ('b22f662d-39d9-551a-ac6c-05fd46d22e22'::uuid, 'CASE-096', 'japanese-american-cultural-community-center', '日美文化与社区中心', 'Japanese American Cultural & Community Center', '日米文化会館', '24032837-3d52-5173-8180-dbe970eec9b8'::uuid, 'kazumi-adachi-kiyoshi-sawano-hideo-matsunaga-isamu-noguchi', 1983, 'published', '洛杉矶', '美国', 'US', 'cultural', 'https://jaccc.org/about/');
...
INSERT INTO image_seed VALUES
  ('31a57674-5697-5c86-b555-c7ed1876df43'::uuid, 'b22f662d-39d9-551a-ac6c-05fd46d22e22'::uuid, 'japanese-american-cultural-community-center', 'CASE-096', '/images/graduation/cases/case-096-japanese-american-community-cultural-center.jpg', 'Another Believer', 'Wikimedia Commons', 'CC BY-SA 4.0', 'https://commons.wikimedia.org/wiki/File:Japanese_American_Cultural_and_Community_Center_%26_Frances_Hashimoto_Plaza.jpg', 'exterior', true);
...
INSERT INTO profile_seed VALUES
  ('CASE-096', 'b22f662d-39d9-551a-ac6c-05fd46d22e22'::uuid, 'japanese-american-cultural-community-center', '文化中心与广场承接族群记忆、表演和公共活动，展示多文化社区如何建立归属感。', NULL, 'Japanese American Community and Cultural Center is a reference case for reading multicultural, communityculture, Design topic: 表演, Design topic: 广场. It can inform thesis thinking about mixed programs, operation, and place-making.', '日系アメリカ人コミュニティの文化拠点。展示、集会、教育を通じて、移民の記憶と公共文化を支える事例として読める。', ARRAY['多文化', '社区文化', '表演', '广场', '族群记忆']::text[], '{}'::text[], ARRAY['multicultural', 'communityculture', 'Design topic: 表演', 'Design topic: 广场', 'Design topic: 族群记忆']::text[], ARRAY['多文化', 'コミュニティ', '文化センター', '公共記憶', '移民']::text[], NULL, NULL, 'https://www.laconservancy.org/learn/historic-places/japanese-american-community-and-cultural-center/', 'published');
...
INSERT INTO assignment_seed VALUES
  ('b22f662d-39d9-551a-ac6c-05fd46d22e22'::uuid, 'japanese-american-cultural-community-center', 'community-center', true, 1.000, 'https://jaccc.org/about/', 'JACCC identifies its Little Tokyo campus as a permanent arts-and-culture community center and confirms Isamu Noguchi''s outdoor plaza. JACCC records the Aratani Theatre as an 880-seat performing-arts venue opened in 1983. LA Conservancy records the 1978–1983 complex, the Center Building by Kazumi Adachi, Kiyoshi Sawano and Hideo Matsunaga, and the adjacent plaza by Isamu Noguchi.', '2026-07-12T00:00:00Z'::timestamptz),
  ('b22f662d-39d9-551a-ac6c-05fd46d22e22'::uuid, 'japanese-american-cultural-community-center', 'theatre', false, 1.000, 'https://jaccc.org/about/', 'JACCC identifies its Little Tokyo campus as a permanent arts-and-culture community center and confirms Isamu Noguchi''s outdoor plaza. JACCC records the Aratani Theatre as an 880-seat performing-arts venue opened in 1983. LA Conservancy records the 1978–1983 complex, the Center Building by Kazumi Adachi, Kiyoshi Sawano and Hideo Matsunaga, and the adjacent plaza by Isamu Noguchi.', '2026-07-12T00:00:00Z'::timestamptz),
  ('b22f662d-39d9-551a-ac6c-05fd46d22e22'::uuid, 'japanese-american-cultural-community-center', 'mixed-use', false, 1.000, 'https://jaccc.org/about/', 'JACCC identifies its Little Tokyo campus as a permanent arts-and-culture community center and confirms Isamu Noguchi''s outdoor plaza. JACCC records the Aratani Theatre as an 880-seat performing-arts venue opened in 1983. LA Conservancy records the 1978–1983 complex, the Center Building by Kazumi Adachi, Kiyoshi Sawano and Hideo Matsunaga, and the adjacent plaza by Isamu Noguchi.', '2026-07-12T00:00:00Z'::timestamptz);
...
BEGIN
  IF (SELECT count(*) FROM architect_seed) <> 1
    ...
    OR (SELECT count(*) FROM assignment_seed) <> 3 THEN
    RAISE EXCEPTION 'Graduation community civic batch 001 seed count mismatch';
  END IF;
  ...
  WHERE slug = ANY (ARRAY['community-center', 'theatre', 'mixed-use']::text[]) AND is_active = true;
  ...
  INTO conflicts;
  IF conflicts <> 0 THEN
    RAISE EXCEPTION 'Graduation community civic batch 001 requires empty target keys; found % conflicts', conflicts;
  END IF;
END $$;
...
BEGIN
  IF (SELECT count(*) FROM public.architects target JOIN architect_seed seed USING (id, slug) WHERE seed.is_new) <> 1
    ...
    OR (SELECT count(*) FROM public.building_function_assignments target JOIN assignment_seed seed USING (building_id, function_slug) WHERE target.review_status = 'approved') <> 3
    ...
    RAISE EXCEPTION 'Graduation community civic batch 001 post-write verification failed';
  END IF;
END $$;
