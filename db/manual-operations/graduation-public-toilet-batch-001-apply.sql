-- Graduation public toilet batch 001: reviewed canonical entities and relations.
-- Generated by scripts/prepare-graduation-library-batch-002.mjs (GRADUATION_REVIEW_BATCH=public-toilet-001).
...
BEGIN
  IF to_regclass('public.graduation_case_profiles') IS NULL
    ...
    RAISE EXCEPTION 'Graduation public toilet batch 001 prerequisites are missing';
  END IF;
END $$;
...
INSERT INTO building_seed VALUES
  ('3e94d879-84d3-5cb7-934f-fa08bf50e0d7'::uuid, 'CASE-044', 'nabeshima-shoto-park-toilet', '锅岛松涛公园厕所', 'Nabeshima Shoto Park Toilet', '鍋島松濤公園トイレ', '51797239-be38-4dbd-9d7e-e413ddf3c78a'::uuid, 'kengo-kuma', 2021, 'published', '东京', '日本', 'JP', 'civic-public', 'https://tokyotoilet.jp/en/nabesima_park/');
...
INSERT INTO image_seed VALUES
  ('3dcb24c5-bdad-53d3-830d-6dfe615d06eb'::uuid, '3e94d879-84d3-5cb7-934f-fa08bf50e0d7'::uuid, 'nabeshima-shoto-park-toilet', 'CASE-044', '/images/graduation/cases/case-044-nabeshima-shoto-park-toilet.jpg', '鋸香具師', 'Wikimedia Commons', 'CC BY-SA 4.0', 'https://commons.wikimedia.org/wiki/File:Shoto_park_2302.jpg', 'exterior', true);
...
INSERT INTO profile_seed VALUES
  ('CASE-044', '3e94d879-84d3-5cb7-934f-fa08bf50e0d7'::uuid, 'nabeshima-shoto-park-toilet', '五个小木屋式公共厕所散落在公园中，以步道和木百叶改善厕所的安全感和亲近感。', NULL, 'Nabeshima Shoto Park Toilet is a reference case for reading public toilet, park, Design topic: 木材, safety. It can inform thesis thinking about mixed programs, operation, and place-making.', '「Nabeshima Shoto Park Toilet」は、公共トイレ・公園・木材・安全の関係を読み取る参考事例。用途の複合、運営、居場所のつくり方を卒業設計に転用して考えられる。', ARRAY['公共厕所', '公园', '木材', '安全', '步行']::text[], '{}'::text[], ARRAY['public toilet', 'park', 'Design topic: 木材', 'safety', 'Design topic: 步行']::text[], ARRAY['公共トイレ', '公園', '木材', '安全', '歩行']::text[], NULL, NULL, 'https://tokyotoilet.jp/en/nabesima_park/', 'published');
...
INSERT INTO assignment_seed VALUES
  ('3e94d879-84d3-5cb7-934f-fa08bf50e0d7'::uuid, 'nabeshima-shoto-park-toilet', 'public-toilet', true, 1.000, 'https://tokyotoilet.jp/en/nabesima_park/', 'THE TOKYO TOILET official facility page identifies Nabeshima Shoto Park, 2-10-7 Shoto, Kengo Kuma and an opening date of June 24, 2021. Kengo Kuma and Associates publishes a dedicated Nabeshima Shoto Park Toilet project page and identifies the exact built facility.', '2026-07-12T00:00:00Z'::timestamptz),
  ('3e94d879-84d3-5cb7-934f-fa08bf50e0d7'::uuid, 'nabeshima-shoto-park-toilet', 'public-space', false, 1.000, 'https://tokyotoilet.jp/en/nabesima_park/', 'THE TOKYO TOILET official facility page identifies Nabeshima Shoto Park, 2-10-7 Shoto, Kengo Kuma and an opening date of June 24, 2021. Kengo Kuma and Associates publishes a dedicated Nabeshima Shoto Park Toilet project page and identifies the exact built facility.', '2026-07-12T00:00:00Z'::timestamptz);
...
BEGIN
  IF (SELECT count(*) FROM architect_seed) <> 1
    ...
    RAISE EXCEPTION 'Graduation public toilet batch 001 seed count mismatch';
  END IF;
  ...
  WHERE slug = ANY (ARRAY['public-toilet', 'public-space']::text[]) AND is_active = true;
  SELECT count(*) INTO required_type_matches
  FROM public.building_types
  WHERE slug = ANY (ARRAY['civic-public']::text[]);
  ...
  INTO conflicts;
  IF conflicts <> 0 THEN
    RAISE EXCEPTION 'Graduation public toilet batch 001 requires empty target keys; found % conflicts', conflicts;
  END IF;
END $$;
...
BEGIN
  IF (SELECT count(*) FROM public.architects target JOIN architect_seed seed USING (id, slug) WHERE seed.is_new) <> 0
    ...
    RAISE EXCEPTION 'Graduation public toilet batch 001 post-write verification failed';
  END IF;
END $$;
