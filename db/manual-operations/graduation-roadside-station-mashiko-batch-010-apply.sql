-- Graduation Roadside Station Mashiko batch 010: reviewed canonical entities and relations.
-- Generated by scripts/prepare-graduation-library-batch-002.mjs (GRADUATION_REVIEW_BATCH=roadside-station-mashiko-010).
...
BEGIN
  IF to_regclass('public.graduation_case_profiles') IS NULL
    ...
    RAISE EXCEPTION 'Graduation Roadside Station Mashiko batch 010 prerequisites are missing';
  END IF;
END $$;
...
INSERT INTO architect_seed VALUES
  ('a198873f-8ff8-5d45-bdc3-a803925f0afe'::uuid, 'mount-fuji-architects-studio', 'MOUNT FUJI ARCHITECTS STUDIO', 'MOUNT FUJI ARCHITECTS STUDIO', 'MOUNT FUJI ARCHITECTS STUDIO', 'https://www.fuji-studio.jp/', true);
...
INSERT INTO building_seed VALUES
  ('c46006a0-e598-501a-a87d-ea00bdb898a9'::uuid, 'CASE-043', 'roadside-station-mashiko', '道之驿益子', 'Roadside Station Mashiko', '道の駅ましこ', 'a198873f-8ff8-5d45-bdc3-a803925f0afe'::uuid, 'mount-fuji-architects-studio', 2016, 'published', '益子', '日本', 'JP', 'transportation', 'https://www.fuji-studio.jp/index_E.html#section_works_Mashiko');
...
INSERT INTO image_seed VALUES
  ('ccbe5a95-6566-5129-9581-476108e12699'::uuid, 'c46006a0-e598-501a-a87d-ea00bdb898a9'::uuid, 'roadside-station-mashiko', 'CASE-043', '/images/graduation/cases/case-043-roadside-station-mashiko.jpg', 'アラツク', 'Wikimedia Commons', 'CC BY-SA 4.0', 'https://commons.wikimedia.org/wiki/File:Michinoeki_Mashiko.jpg', 'exterior', true);
...
INSERT INTO profile_seed VALUES
  ('CASE-043', 'c46006a0-e598-501a-a87d-ea00bdb898a9'::uuid, 'roadside-station-mashiko', '地方木材和土形成道之驿大屋顶，把农产销售、休息和地方象征整合在一起。', NULL, 'Roadside Station Mashiko is a reference case for reading roadside station, local food, Design topic: local木材, transport. It can inform thesis thinking about mixed programs, operation, and place-making.', '道の駅を地域食、地域木材、交通の結節点として設計した事例。地域の入口となる小さな公共建築のつくり方を比較できる。', ARRAY['道之驿', '地域食', '地方木材', '交通', '地方服务']::text[], '{}'::text[], ARRAY['roadside station', 'local food', 'Design topic: local木材', 'transport', 'localservice']::text[], ARRAY['道の駅', '地域食', '地域木材', '交通', '地域サービス']::text[], NULL, NULL, 'https://www.fuji-studio.jp/index_E.html#section_works_Mashiko', 'published');
...
INSERT INTO assignment_seed VALUES
  ('c46006a0-e598-501a-a87d-ea00bdb898a9'::uuid, 'roadside-station-mashiko', 'transport-hub', true, 1.000, 'https://www.fuji-studio.jp/index_E.html#section_works_Mashiko', 'MOUNT FUJI ARCHITECTS STUDIO identifies the project as Roadside Station in Mashiko, Tochigi, completed in 2016, with roadside station as its principal use. The official project data records one storey, 1,328.84 square metres total floor area and construction from September 2015 to September 2016. The official roadside-station operator and Kanto Michi-no-Eki listing confirm the Nagatsutsumi 2271 address, parking, restaurant and retail operation. These support transport-hub primary and retail secondary without expanding the building to community-center or public-space.', '2026-07-12T00:00:00Z'::timestamptz),
  ('c46006a0-e598-501a-a87d-ea00bdb898a9'::uuid, 'roadside-station-mashiko', 'retail', false, 1.000, 'https://www.fuji-studio.jp/index_E.html#section_works_Mashiko', 'MOUNT FUJI ARCHITECTS STUDIO identifies the project as Roadside Station in Mashiko, Tochigi, completed in 2016, with roadside station as its principal use. The official project data records one storey, 1,328.84 square metres total floor area and construction from September 2015 to September 2016. The official roadside-station operator and Kanto Michi-no-Eki listing confirm the Nagatsutsumi 2271 address, parking, restaurant and retail operation. These support transport-hub primary and retail secondary without expanding the building to community-center or public-space.', '2026-07-12T00:00:00Z'::timestamptz);
...
BEGIN
  IF (SELECT count(*) FROM architect_seed) <> 1
    ...
    RAISE EXCEPTION 'Graduation Roadside Station Mashiko batch 010 seed count mismatch';
  END IF;
  ...
  WHERE slug = ANY (ARRAY['transport-hub', 'retail']::text[]) AND is_active = true;
  ...
  INTO conflicts;
  IF conflicts <> 0 THEN
    RAISE EXCEPTION 'Graduation Roadside Station Mashiko batch 010 requires empty target keys; found % conflicts', conflicts;
  END IF;
END $$;
...
BEGIN
  IF (SELECT count(*) FROM public.architects target JOIN architect_seed seed USING (id, slug) WHERE seed.is_new) <> 1
    ...
    RAISE EXCEPTION 'Graduation Roadside Station Mashiko batch 010 post-write verification failed';
  END IF;
END $$;
