...
## Runtime Fallback Content（P0 覆盖层）

当数据库暂时没有建筑作品正文或建筑师本地化简介时，前台允许使用事实型 fallback，目的只是避免空页面，不等同于精品内容。

允许：

- 使用已存在字段组织说明：作者、年份、地点、国家、时代、风格、类型、结构、面积、作品数量
- 明确写成“如何阅读这个档案节点”的导引
- 对空间、光线、动线使用观察提示，而不是未经来源验证的具体结论
- 中文/日文缺简介时使用本地化事实摘要，避免直接露出英文短句

禁止：

- 把 fallback 写成确定的历史评价，例如“开创了某某主义”，除非该事实来自已审校内容或来源
- 用 fallback 冒充长文、研究文章或人工审校结论
- 在没有可靠来源时编造建筑师师承、影响关系、材料体系、争议评价
- 用 Unsplash 或来源不清图片作为人物肖像

质量标准：

- fallback 文案必须可读、诚实、克制；它是“档案入口”，不是“最终内容”
- 精品页仍必须使用来源驱动的 overlay：中日正文、代表作导读、来源列表和授权清晰图片
- 无可靠人物肖像时，可以暂不展示本人照片；若用代表作图像兜底，alt 和说明必须明确写为“代表作图像”
- 每次批量补内容后运行 `npm run content:audit-display`，确认前台真实展示缺口；仍缺图的建筑师必须记录原因，不能用来源不清或授权不明图片填充

## Building Content Overlay（建筑作品研究卡）

建筑作品正式内容优先使用仓库 overlay：`src/lib/building-content.ts`。该层用于替代运行时 fallback，仍不直接写 Supabase。

第一轮标准：

- 每座建筑必须有中文和日文 `summary`、`significance`、3 个研究章节和 `sources`
- `summary` 控制在约 120-180 中文字；日文保持等量信息密度
- 3 个章节固定围绕：时代与委托背景、设计思想与空间组织、材料/光线与历史影响
- 每座建筑至少 2 条来源，URL 必须可打开
- 英文只作为回退字段，不作为本轮质量目标

禁止：

- 把 fallback 文案复制进 overlay 充数
- 使用“第一版”“已审校”“待补齐”等内部工作口吻
- 对有争议的政治或历史语境只写形式美感，忽略必要背景
- 因图片不足而编造材料、结构、委托人或评价
