...
第二批精品内容新增 `le-corbusier`、`mies-van-der-rohe`、`frank-lloyd-wright`、`louis-kahn`、`tadao-ando`。每个代表作 slug 必须能被 `getBuildingsWithCovers()` 解析到站内建筑，并应有 Supabase 主图或审核后的本地/远程图片 override。
