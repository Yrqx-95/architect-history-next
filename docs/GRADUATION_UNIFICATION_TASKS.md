...
2026-07-14 后续人工审核发现 Library batch 002 的 CASE-130 Book Mountain 官方链接在建筑、profile、3 条 function assignment 和 compatibility payload 中被统一截断为 `.../book-`。独立修正批 `architect-intro-p0-data-001` 已建立 reviewed decision、guarded migration、rollback 和隔离验证器；它同时修复 Grafton / UNEMORI 身份、两条 Grafton 建筑数据，并拒绝 Town House 的 3 张错配 Unsplash 图片。该批当前尚未写入生产，详见 `ARCHITECT_INTRO_P0_DATA_001.md`。
