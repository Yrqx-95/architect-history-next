...
Library batch 002 已完成 15 条逐项审核：14 条通过，CASE-079 保留为 `no_safe_image_yet`。版本化决定、guarded apply、dependency-aware rollback 和 PGlite 干跑包已生成，范围为 11 个新 architect、14 buildings、14 primary images、14 profiles 和 36 assignments。干跑已按 foundation → unification batch 001 → library batch 001 → library batch 002 完整重放，两轮 apply/rollback 和外部关系拒绝保护均通过。生产只读预检为 0 个 architect/building/image/profile/assignment 目标冲突，但尚未写入。当前 G6 仍是 8/118 已生产迁移、22/118 已审核；library 子集为 8/23 已迁移、22/23 已审核。详见 `GRADUATION_NEW_BUILDING_LIBRARY_002_REVIEW.md` 与 `GRADUATION_LIBRARY_BATCH_002_DRY_RUN.md`。
