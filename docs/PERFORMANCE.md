...
|------|------|------|
| 静态生成页面 | 3,195 | ✅ |
| Vercel 部署包 | 约 43–44MB | ⚠️ 偏大（主要为 curated images） |
| Client Components | 12 个 | ✅ 合理 |
| Dynamic imports | 0 | ⚠️ 无代码分割 |
| 最大单文件图片 | 约 999.5KB (JPG) | ✅ 已低于 1MB |
| 首页 TSX 行数 | 500 | ⚠️ 文案已集中，仍需拆组件 |
...
| curated 目录过大 | 220 文件约 75MB，全部部署到 Vercel | 每次部署上传约 43–44MB |
...
3. **压缩超大 JPG**：1MB 以上本地缓存 JPG 已清零；后续新增图片继续保持该阈值
...
| 问题 | 位置 | 影响 |
|------|------|------|
| 首页 500 行 | `src/app/[lang]/page.tsx` | 文案已集中，但单体页面仍偏大，难以维护 |
