...
- 时间轴、地图、档案索引等知识导航页优先使用稳定 grid，不用内容高度差过大的瀑布列制造不可预测的空白
- 侧栏或索引列不得比主内容长出大段无信息区域；若数据密度不够，拆成独立 section 或转为紧凑列表
- 时间轴页需要同时提供叙事层与快速定位层；横向年代导航可拖动浏览，并通过锚点进入具体年代，避免用户只能长距离纵向滚动
- 时间轴页必须把时代作为“问题链”呈现：每个时代至少说明一个核心建筑史问题，以及进入下一时代的历史转向
- 时代详情页不能只显示统计数字和列表；首屏下方必须承接时间轴问题模型，显示该时代的核心问题、历史转向和关键词，并提供返回时间轴对应阶段的入口
- 风格详情页若有关联时代，也必须显示简短时代背景：这个风格回应了什么建筑史问题、处于哪次历史转向中，并提供时代页与时间轴入口
- 建筑详情页不能只以图片、metadata 和空间/光线段落构成；正文分析前必须说明作品所处历史问题、时代转向，以及进入时代页/时间轴的路径
- 图谱页必须优先可读性，不用无标签的大型力导向图作为第一屏；第一版应呈现清晰谱系路径、关系索引、关系类型、方向、说明和来源
...
- 分类页中没有可信图片的建筑不要强行展示大面积图片占位；应进入 `文字索引`，保留名称、年份、地点和作者信息
- 地图页的国家/地区入口属于知识导航，不是作品展示；封面图应使用缩略高度，避免单个地域卡片在移动端占据过多首屏
...
- 暗色模式不得把建筑照片整体压暗、降饱和或套用全局混合模式；只允许在文字覆盖区域使用局部渐变，且必须保留建筑原图的亮度、天空和材质观感
- 首页 hero 移动端文字与内框、图片边缘必须保留清晰呼吸感；标题、metadata 和署名之间不得贴边堆叠
...
分类页是档案入口，不是数据库字段清单。用户点击一个分类前，应能看出里面有真实内容、它与其他路径的关系，以及继续阅读的方向。

### 规则

- 分类页必须按“建筑师、作品、时代/风格/类型、地域”分区，不把所有名称无序平铺。
- 有内容的入口才展示；若某个时代、风格、类型没有建筑师也没有建筑作品，不在主列表中显示。
- 计数不得展示 `0 建筑师`、`0 建筑` 这类失望信息；某一维度为 0 时省略该维度，只保留真实可进入的内容数量。
- 旧数据中 `type_slug`、`era_slug`、`style_slugs` 可能存了中文/英文显示名；前台分类统计必须兼容 slug、name_en、name_zh、name_ja。
- 分区说明必须是面向用户的编辑文案，禁止出现“这里不再把名字平铺堆叠”等内部实现口吻。
- 移动端分类卡片要有明确 surface、border、标题、metadata 层级，避免长列表像系统表格或脚手架页面。
...
- 移动端导航优先使用底部抽屉或清晰全屏面板；不得出现侧边露出旧页面、重复关闭按钮、设置项过度拉长的半开抽屉观感
...
- 分类浏览页必须先给“路径”和“层级”，再给列表：建筑师谱系、建筑作品索引、历史/风格/类型、地域入口应分区呈现，避免把全部名称无序平铺
- 分类详情页的统计、关联分类、建筑师、建筑作品应使用清晰分段；建筑师列表使用有 surface/border 的索引卡，不再使用纯文本堆叠
- 分类详情页的建筑师和作品列表应优先使用 masonry/columns，尤其是有图作品卡和长标题卡；统计卡必须放入正常 section 节奏，不要孤立贴在 header 下方造成断层
- 地域/地图入口必须先提供真实可进入内容：国家/地区密度、城市线索、代表作品图和明确链接；不要使用空地图 SDK、0 计数列表或“即将开放”式占位来消耗用户期待
- 国家/地区总页必须和地图页共享 archive index 逻辑：总览指标、密度条、可信本地封面矩阵或城市文字索引、明确的地域详情入口
- 地图页与地域列表必须本地化国家/地区名；英文和日文页面不得把中文城市、中文国家名或中文简繁控件作为 fallback 展示给用户
- 地图可视化只有在经纬度覆盖、移动端性能和版权/署名都可控时再升级；第一层体验优先做成可读、可点、可继续探索的 archive index
- 地图、时间、档案这类功能页必须采用“总览指标 + 密集索引 + 可直接进入的内容卡”结构；避免单条竖线时间轴、超高右侧面板、被拉伸的卡片或大面积空白列
- 入口型图片只使用本地已治理图片；远程图、破图、低辨识度图或非建筑主体图不得作为地图/档案索引封面，缺图时使用克制文字占位
- 时间页必须保留三层阅读结构：`Period navigator` 负责快速跳到大历史时期，`Narrative timeline` 负责历史连续性，`Decade atlas` 负责高密度查找；不要只保留卡片索引，也不要只保留稀疏长线
- 时间、地图、档案页遇到内容高度不一致的索引卡时，优先使用 masonry/columns 或分段重排；避免 CSS grid 等高行把短卡片撑成大空白
- 地图页、时间页、档案页的索引型卡片若内容长短不一，必须使用 `columns + break-inside-avoid` 或拆分为图片卡/文字索引两层；不要把缺图城市、短年代、短分类硬塞进等高图片卡
- 地图页的国家/地区入口必须提供“为什么从这里读”的短路径文案；仅有国家名、计数和进度条会让页面退化成数据库索引
- 地图页国家/地区入口不应使用过大的图片区；代表图只作为路径提示，手机端单张国家卡应尽量控制在约 240-320px 高度内
- 建筑师详情页的作品区不能只把图片卡和文字索引并排放置；左侧/旁侧区域必须承担年表、时代、风格、地域或时间入口，避免形成装饰性空栏
- 建筑师详情页的关联建筑师 fallback 应使用紧凑索引行；只有在卡片内有肖像、关系说明或丰富摘要时才使用大卡片
- 建筑作品详情页必须按“历史背景 → 作品研究地图 → 空间/光线/动线分析 → 结构材料 → 来源 → 关联路径”组织；研究地图可以显示待补充状态，但待补充项不得链接到空 section，也不得生成大面积空占位
- 建筑作品详情页的技术字段必须按语言过滤：日文页不得直接展示未本地化的中文结构、材料或说明文本；缺少本地化时宁可显示待补充状态
- 建筑作品详情页图库必须过滤失效外部图片、SVG 原图和 Next 图片优化无法处理的资源；若 Supabase 图片不可展示，应回退到本地已治理 curated cover，而不是展示破图或大面积占位
