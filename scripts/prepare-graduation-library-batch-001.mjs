...
const OUTPUT_JSON = path.join(ROOT, 'db/review-packets/graduation-library-batch-001.json')
const APPLY_SQL = path.join(ROOT, 'db/manual-operations/graduation-library-batch-001-apply.sql')
const ROLLBACK_SQL = path.join(ROOT, 'db/manual-operations/graduation-library-batch-001-rollback.sql')
...
const architectDrafts = {
  'coelacanth-kh': {
    name_zh: 'Coelacanth K&H建筑事务所',
    name_en: 'Coelacanth K&H Architects',
    name_ja: 'シーラカンスK&H',
    official_url: 'http://www.coelacanth-kandh.co.jp/',
  },
  'kw-hg-architects': {
    name_zh: 'kw+hg建筑事务所',
    name_en: 'kw+hg architects',
    name_ja: 'kw+hgアーキテクツ',
    official_url: 'https://kwhg.co.jp/',
  },
  'mari-ito-uao': {
    name_zh: '伊藤麻理 + UAo',
    name_en: 'Mari Ito + UAo',
    name_ja: '伊藤麻理 + UAo',
    official_url: 'https://www.u-a-o.jp/',
  },
  'mikami-architects': {
    name_zh: '三上建筑事务所',
    name_en: 'MIKAMI Architects',
    name_ja: '三上建築事務所',
    official_url: 'https://mikami-arc.co.jp/',
  },
}
...
assert(decisions.decisions.length === 8, `Expected 8 decisions, found ${decisions.decisions.length}`)
...
const images = decisions.decisions.map(item => {
  ...
  return {
    ...
    img_type: item.case_id === 'CASE-027' ? 'interior' : 'exterior',
    is_primary: true,
  }
})
...
assert(new Set(buildings.map(item => item.id)).size === 8, 'Building UUID collision')
assert(new Set(images.map(item => item.id)).size === 8, 'Image UUID collision')
assert(new Set(profiles.map(item => item.case_id)).size === 8, 'Profile CASE collision')
assert(assignments.length === 17, `Expected 17 assignments, found ${assignments.length}`)
assert(new Set(assignments.map(item => `${item.building_id}:${item.function_slug}`)).size === 17, 'Assignment collision')

const pack = {
  schema_version: 1,
  batch_id: 'graduation-library-batch-001',
...
function buildApplySql(pack) {
  ...
  return `-- Graduation library batch 001: reviewed canonical entities and relations.
...
function buildRollbackSql(pack) {
  ...
  return `-- Rollback graduation library batch 001 only.
