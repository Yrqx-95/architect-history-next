...
const hotelApply = read('db/manual-operations/building-function-hotel-001-apply.sql')
const apply = read('db/manual-operations/building-function-art-center-001-apply.sql')
const rollback = read('db/manual-operations/building-function-art-center-001-rollback.sql')
...
try {
  ...
  if (await count('public.building_function_aliases') !== beforeAliases + 8) throw new Error('forward alias count mismatch')

  const buildingId = [...buildings.keys()][0]
  await db.exec(`INSERT INTO public.building_function_assignments (building_id, function_slug, is_primary, confidence, review_status, assignment_method, evidence_url, evidence_note, reviewed_at) VALUES ('${buildingId}'::uuid, 'art-center', true, 1, 'approved', 'manual', 'https://example.com', 'rollback guard', now());`)
  let refused = false
  try {
  ...
  await db.exec(`DELETE FROM public.building_function_assignments WHERE function_slug = 'art-center';`)
  ...
  console.log('Art-center taxonomy isolated PostgreSQL dry-run passed.')
  console.log(`- PostgreSQL engine: ${await scalar("select current_setting('server_version_num')::integer")}`)
  console.log('- historical order: base taxonomy, public-space, public-toilet, retail, then hotel')
  console.log('- forward: 1 function, 8 aliases across 4 locales')
