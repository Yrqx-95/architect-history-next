...
const apply = read('db/manual-operations/building-function-public-toilet-001-apply.sql')
const rollback = read('db/manual-operations/building-function-public-toilet-001-rollback.sql')
...
try {
  await db.exec(`
    ...
    INSERT INTO public.building_types (slug) VALUES
      ('cultural'), ('educational'), ('civic-public'), ('healthcare'), ('residential'), ('mixed-use'), ('transportation'), ('public-space');
  `)
  ...
  if (await count('public.building_function_aliases') !== beforeAliases + 24) throw new Error('forward alias count mismatch')

  const buildingId = [...buildings.keys()][0]
  await db.exec(`INSERT INTO public.building_function_assignments (building_id, function_slug, is_primary, confidence, review_status, assignment_method, evidence_url, evidence_note, reviewed_at) VALUES ('${buildingId}'::uuid, 'public-toilet', true, 1, 'approved', 'manual', 'https://example.com', 'rollback guard', now());`)
  let refused = false
  try {
  ...
  await db.exec(`DELETE FROM public.building_function_assignments WHERE function_slug = 'public-toilet';`)
  ...
  console.log('Public-toilet taxonomy isolated PostgreSQL dry-run passed.')
  console.log(`- PostgreSQL engine: ${await scalar("select current_setting('server_version_num')::integer")}`)
  console.log('- historical order: base taxonomy then public-space taxonomy')
  console.log('- forward: 1 function, 24 aliases across 4 locales')
