...
const apply = read('db/manual-operations/building-function-retail-001-apply.sql')
const rollback = read('db/manual-operations/building-function-retail-001-rollback.sql')
...
try {
  ...
  await db.exec(`INSERT INTO public.building_function_assignments (building_id, function_slug, is_primary, confidence, review_status, assignment_method, evidence_url, evidence_note, reviewed_at) VALUES ('${buildingId}'::uuid, 'retail', true, 1, 'approved', 'manual', 'https://example.com', 'rollback guard', now());`)
  let refused = false
  try {
  ...
  await db.exec(`DELETE FROM public.building_function_assignments WHERE function_slug = 'retail';`)
  ...
  console.log('Retail taxonomy isolated PostgreSQL dry-run passed.')
  console.log(`- PostgreSQL engine: ${await scalar("select current_setting('server_version_num')::integer")}`)
  console.log('- historical order: base taxonomy, public-space, then public-toilet')
