...
const LE_CORBUSIER_ID = 'fbdda76b-fde9-4203-8b68-475d7e40e09a'
const KUNIO_MAEKAWA_ID = '465a8e38-ae69-4b78-862f-c2b7f1f2e001'
...
try {
  ...
  console.log('- forward: Parc.1 metadata plus exact three unsafe-primary suppression; NMWA canonical metadata and architect_id')
  console.log('- rollback: exact preflight metadata, architect_id, and Parc.1 primary flags restored; NMWA image rows unchanged')
  console.log('- replay refusal: forward replay rejected')
  console.log('- drift refusal: building-field, architect-id, primary-state, extra-row, rollback-building, rollback-architect-id, rollback-text, and rollback-extra-row cases rejected')
} catch (error) {
  console.error(error)
  process.exitCode = 1
}
...
async function runDriftChecks() {
  ...
  const architectIdDrift = await createSeededDb()
  try {
    await architectIdDrift.exec(`UPDATE public.buildings SET architect_id = '${KUNIO_MAEKAWA_ID}'::uuid WHERE id = '${NMWA_ID}'::uuid`)
    const message = await expectRefusal(architectIdDrift, apply, 'architect_id drift')
    if (!message.includes('NMWA building precondition')) throw new Error(`architect_id drift was refused for the wrong reason: ${message}`)
  } finally {
    await architectIdDrift.close()
  }
  ...
  const rollbackArchitectIdDrift = await createSeededDb()
  try {
    await rollbackArchitectIdDrift.exec(apply)
    await rollbackArchitectIdDrift.exec(`UPDATE public.buildings SET architect_id = NULL WHERE id = '${NMWA_ID}'::uuid`)
    const message = await expectRefusal(rollbackArchitectIdDrift, rollback, 'rollback architect_id drift')
    if (!message.includes('NMWA rollback refused')) throw new Error(`rollback architect_id drift was refused for the wrong reason: ${message}`)
  } finally {
    await rollbackArchitectIdDrift.close()
  }
...
async function createSeededDb() {
  ...
  await db.exec(`
    ...
    INSERT INTO public.architects (id, slug) VALUES
      ('${LE_CORBUSIER_ID}'::uuid, 'le-corbusier'),
      ('${KUNIO_MAEKAWA_ID}'::uuid, 'kunio-maekawa'),
      ('deeb138b-3f6a-48ad-9db7-c141b72beb06'::uuid, 'richard-rogers');
    ...
    INSERT INTO public.buildings (id, slug, name_en, name_zh, name_ja, architect_id, architect_slug, year_start, country_code, era_slug, city, country, type_slug, description, significance, official_url, updated_at) VALUES
      ('${PARC_ID}'::uuid, 'parc1', 'Parc1', '', '', NULL, 'richard-rogers', 2020, 'KR', 'contemporary', NULL, NULL, NULL, NULL, NULL, NULL, '2026-07-08T16:11:20.896254+00:00'::timestamptz),
      ('${NMWA_ID}'::uuid, 'national-museum-of-western-art', 'National Museum of Western Art', '', '', NULL, 'kunio-maekawa', 1959, 'JP', NULL, NULL, NULL, 'cultural', NULL, NULL, NULL, '2026-05-24T00:02:34.681443+00:00'::timestamptz);
...
async function assertForward(db) {
  ...
  if (await scalar(db, `SELECT count(*) FROM public.buildings WHERE slug = 'national-museum-of-western-art' AND architect_id = '${LE_CORBUSIER_ID}'::uuid AND architect_slug = 'le-corbusier' AND city = 'Tokyo' AND country = 'Japan' AND era_slug = 'modern' AND description ?& array['zh', 'en', 'ja'] AND significance ?& array['zh', 'en', 'ja']`) !== 1) throw new Error('forward NMWA mismatch')
...
async function assertRollback(db) {
  ...
  if (await scalar(db, `SELECT count(*) FROM public.buildings WHERE slug = 'national-museum-of-western-art' AND name_zh = '' AND name_ja = '' AND architect_id IS NULL AND architect_slug = 'kunio-maekawa' AND city IS NULL AND country IS NULL AND era_slug IS NULL AND description IS NULL AND significance IS NULL AND official_url IS NULL AND updated_at = '2026-05-24T00:02:34.681443+00:00'::timestamptz`) !== 1) throw new Error('rollback NMWA mismatch')
