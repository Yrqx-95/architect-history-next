...
const migration = read('supabase/migrations/20260713134524_duplicate_primary_image_review_003.sql')
const rollback = read('db/manual-operations/duplicate-primary-image-review-003-rollback.sql')
...
try {
  await db.exec(`
    ...
    INSERT INTO public.images SELECT id, building_id, url_original, pre_thumb, photographer, source, license, source_url, img_type, true FROM reviewed_image_seed;
    COMMIT;
  `)
  await db.exec(migration)
  await assertForward()
  let replayRefused = false
  try { await db.exec(migration) } catch (error) { replayRefused = String(error).includes('Reviewed primary image rows changed'); await db.exec('ROLLBACK;') }
  ...
  await db.exec(rollback); await assertRollback()
  console.log('Duplicate primary image review 003 isolated PostgreSQL dry-run passed.')
  ...
  console.log('- guards: exact building identity, all 4 prior rows, extra-primary refusal and replay refusal')
