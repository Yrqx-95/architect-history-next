...
const migration = read('supabase/migrations/20260713143457_duplicate_primary_image_review_004.sql')
const rollback = read('db/manual-operations/duplicate-primary-image-review-004-rollback.sql')
...
try {
  ...
  await db.exec(rollback); await assertRollback()
  console.log('Duplicate primary image review 004 isolated PostgreSQL dry-run passed.')
  console.log(`- PostgreSQL engine: ${await scalar("select current_setting('server_version_num')::integer")}`)
  console.log('- forward: 4 buildings reduced from 8 primary rows to 4 reviewed Commons primary rows')
...
async function assertForward() {
  if (await scalar('select count(*) from public.images where is_primary') !== 4) throw new Error('Forward primary count mismatch')
  if (await scalar("select count(*) from public.images where source = 'Wikimedia Commons' and is_primary") !== 4) throw new Error('Forward source mismatch')
}
async function assertRollback() { if (await scalar('select count(*) from public.images where is_primary') !== 8) throw new Error('Rollback primary count mismatch') }
