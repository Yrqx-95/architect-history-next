...
const pack = readJson('db/review-packets/graduation-library-batch-001.json')
...
const applySql = readText('db/manual-operations/graduation-library-batch-001-apply.sql')
const rollbackSql = readText('db/manual-operations/graduation-library-batch-001-rollback.sql')
...
try {
  ...
  const existingArchitects = pack.architects.filter(item => !item.is_new)
  ...
  console.log('Graduation library batch 001 isolated PostgreSQL dry-run passed.')
...
async function assertForwardState(phase) {
  const expected = {
    architects: pack.counts.architects,
    buildings: baseBuildingCount + pack.counts.buildings,
    images: pack.counts.images,
    profiles: basePack.counts.profiles + pack.counts.profiles,
    functions: basePack.counts.functions,
    aliases: basePack.counts.aliases,
    assignments: basePack.counts.assignments + pack.counts.assignments,
    primary_images: pack.counts.images,
    primary_functions: pack.counts.buildings,
  }
...
async function assertBaseState(phase) {
  await assertCounts({
    architects: pack.counts.architects - pack.counts.new_architects,
    buildings: baseBuildingCount,
    images: 0,
    profiles: basePack.counts.profiles,
    functions: basePack.counts.functions,
    aliases: basePack.counts.aliases,
    assignments: basePack.counts.assignments,
    primary_images: 0,
    primary_functions: 0,
  }, phase)
}
