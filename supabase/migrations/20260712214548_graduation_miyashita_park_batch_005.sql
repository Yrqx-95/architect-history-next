-- Graduation Miyashita Park batch 005: reviewed canonical entities and relations.
-- Generated by scripts/prepare-graduation-library-batch-002.mjs (GRADUATION_REVIEW_BATCH=miyashita-park-005).
...
BEGIN
  IF to_regclass('public.graduation_case_profiles') IS NULL
    ...
    RAISE EXCEPTION 'Graduation Miyashita Park batch 005 prerequisites are missing';
  END IF;
END $$;
...
INSERT INTO architect_seed VALUES
  ('49dee726-4fa3-5f25-a7d0-0a0dfb2a8d71'::uuid, 'takenaka-corporation-nikken-sekkei', '竹中工务店 + 日建设计', 'Takenaka Corporation + Nikken Sekkei', '竹中工務店 + 日建設計', 'https://www.nikken.co.jp/en/projects/urban_design_and_planning/miyashita_park.html', true);
...
INSERT INTO building_seed VALUES
  ('2558b8fa-1cb6-55f4-9f32-710d3005ea74'::uuid, 'CASE-040', 'miyashita-park', '宫下公园', 'MIYASHITA PARK', 'MIYASHITA PARK', '49dee726-4fa3-5f25-a7d0-0a0dfb2a8d71'::uuid, 'takenaka-corporation-nikken-sekkei', 2020, 'published', '东京', '日本', 'JP', 'mixed-use', 'https://www.nikken.co.jp/en/projects/urban_design_and_planning/miyashita_park.html');
...
INSERT INTO image_seed VALUES
  ('b442816c-953f-5f0c-9e79-186c0b2c5f6a'::uuid, '2558b8fa-1cb6-55f4-9f32-710d3005ea74'::uuid, 'miyashita-park', 'CASE-040', '/images/graduation/cases/case-040-miyashita-park.jpg', 'Nesnad', 'Wikimedia Commons', 'CC BY 4.0', 'https://commons.wikimedia.org/wiki/File:Miyashita_Park_2025_May_26_various_01.jpg', 'exterior', true);
...
INSERT INTO profile_seed VALUES
  ('CASE-040', '2558b8fa-1cb6-55f4-9f32-710d3005ea74'::uuid, 'miyashita-park', '屋上公园、商业和酒店叠合，重新讨论高密度城市中公共空间与商业的关系。', NULL, 'MIYASHITA PARK is a reference case for reading Design topic: 屋上park, Design topic: 商业复合, Design topic: 运动, publicness. It can inform thesis thinking about mixed programs, operation, and place-making.', '「MIYASHITA PARK」は、屋上公園・商業複合・運動・公共性の関係を読み取る参考事例。用途の複合、運営、居場所のつくり方を卒業設計に転用して考えられる。', ARRAY['屋上公园', '商业复合', '运动', '公共性', '无障碍']::text[], '{}'::text[], ARRAY['Design topic: 屋上park', 'Design topic: 商业复合', 'Design topic: 运动', 'publicness', 'Design topic: 无障碍']::text[], ARRAY['屋上公園', '商業複合', '運動', '公共性', 'バリアフリー']::text[], NULL, NULL, 'https://www.nikken.co.jp/en/projects/urban_design_and_planning/miyashita_park.html', 'published');
...
INSERT INTO assignment_seed VALUES
  ('2558b8fa-1cb6-55f4-9f32-710d3005ea74'::uuid, 'miyashita-park', 'mixed-use', true, 1.000, 'https://www.nikken.co.jp/en/projects/urban_design_and_planning/miyashita_park.html', 'Nikken Sekkei identifies MIYASHITA PARK as the 2020 Shibuya PPP reconstruction with park, parking, four retail floors and an eighteen-storey hotel. The official role table identifies Takenaka Corporation as lead architect and Nikken Sekkei as project architect.', '2026-07-12T00:00:00Z'::timestamptz),
  ('2558b8fa-1cb6-55f4-9f32-710d3005ea74'::uuid, 'miyashita-park', 'retail', false, 1.000, 'https://www.nikken.co.jp/en/projects/urban_design_and_planning/miyashita_park.html', 'Nikken Sekkei identifies MIYASHITA PARK as the 2020 Shibuya PPP reconstruction with park, parking, four retail floors and an eighteen-storey hotel. The official role table identifies Takenaka Corporation as lead architect and Nikken Sekkei as project architect.', '2026-07-12T00:00:00Z'::timestamptz),
  ('2558b8fa-1cb6-55f4-9f32-710d3005ea74'::uuid, 'miyashita-park', 'public-space', false, 1.000, 'https://www.nikken.co.jp/en/projects/urban_design_and_planning/miyashita_park.html', 'Nikken Sekkei identifies MIYASHITA PARK as the 2020 Shibuya PPP reconstruction with park, parking, four retail floors and an eighteen-storey hotel. The official role table identifies Takenaka Corporation as lead architect and Nikken Sekkei as project architect.', '2026-07-12T00:00:00Z'::timestamptz);
...
BEGIN
  IF (SELECT count(*) FROM architect_seed) <> 1
    ...
    RAISE EXCEPTION 'Graduation Miyashita Park batch 005 seed count mismatch';
  END IF;
  ...
  WHERE slug = ANY (ARRAY['mixed-use', 'retail', 'public-space']::text[]) AND is_active = true;
  ...
  INTO conflicts;
  IF conflicts <> 0 THEN
    RAISE EXCEPTION 'Graduation Miyashita Park batch 005 requires empty target keys; found % conflicts', conflicts;
  END IF;
END $$;
...
BEGIN
  IF (SELECT count(*) FROM public.architects target JOIN architect_seed seed USING (id, slug) WHERE seed.is_new) <> 1
    ...
    RAISE EXCEPTION 'Graduation Miyashita Park batch 005 post-write verification failed';
  END IF;
END $$;
