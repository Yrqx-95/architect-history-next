-- Reviewed content-trust package for Parc.1 and the National Museum of Western Art.
-- Production apply is intentionally out of scope for this PR.
-- Guards are pinned to the 2026-07-15 anon-only preflight snapshot.
...
DECLARE
  ...
  nmwa_primary_ids uuid[] := ARRAY[
    'cbf9a81a-c87d-51d2-8da0-954cce0d7f5e'::uuid,
    'f8a78374-d972-5da2-8d2f-9204a7a68310'::uuid
  ];
BEGIN
  IF (SELECT count(*) FROM public.buildings
      WHERE id = parc_id AND slug = 'parc1'
        AND name_zh = '' AND name_ja = ''
        AND architect_slug = 'richard-rogers'
        AND country_code = 'KR' AND era_slug = 'contemporary'
        AND city IS NULL AND country IS NULL AND type_slug IS NULL
    ...
    RAISE EXCEPTION 'Parc.1 building precondition drifted from anon preflight';
  END IF;

  IF (SELECT count(*) FROM public.images WHERE building_id = parc_id) <> 30
     OR (SELECT count(*) FROM public.images WHERE building_id = parc_id AND is_primary) <> 3
     OR (SELECT count(*) FROM public.images WHERE building_id = parc_id AND id = ANY(parc_unsafe_ids) AND is_primary) <> 3 THEN
    RAISE EXCEPTION 'Parc.1 image precondition drifted: expected 30 rows and exactly the three reviewed unsafe primaries';
  END IF;
  ...
  IF (SELECT count(*) FROM public.images WHERE building_id = nmwa_id) <> 6
     OR (SELECT count(*) FROM public.images WHERE building_id = nmwa_id AND is_primary) <> 2
     OR (SELECT count(*) FROM public.images WHERE building_id = nmwa_id AND id = ANY(nmwa_primary_ids) AND is_primary) <> 2 THEN
    RAISE EXCEPTION 'NMWA image precondition drifted: expected six rows and the same two duplicate primaries';
  END IF;
END $$;
...
SET
  ...
  country = 'South Korea',
  type_slug = 'mixed-use',
  official_url = 'https://rshp.com/projects/mixed-use/parc-1/',
...
SET
  ...
  country = 'Japan',
  era_slug = 'modern',
  official_url = 'https://www.nmwa.go.jp/en/about/building.html',
  description = jsonb_build_object(
...
BEGIN
  IF (SELECT count(*) FROM public.buildings
      WHERE id = '39a3d5b5-0308-47e3-b2fe-aebb164353bf'::uuid
        ...
        AND significance ?& array['zh', 'en', 'ja']) <> 1 THEN
    RAISE EXCEPTION 'Parc.1 content postcondition failed';
  END IF;
