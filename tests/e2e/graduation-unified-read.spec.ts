...
test.describe('graduation Supabase + JSON dual read', () => {
  test('API exposes the reviewed unified subset and explicit image fallback diagnostics', async ({ request }) => {
    ...
    expect(payload.diagnostics.profileCount).toBe(79)
    expect(payload.diagnostics.unifiedCaseIds).toHaveLength(79)
    ...
    expect(payload.diagnostics.fallbackImageCaseIds).toHaveLength(79)
    ...
    const miyashitaCase = payload.cases.find((item: { id: string }) => item.id === 'CASE-040')
    expect(miyashitaCase).toMatchObject({
      name: '宫下公园',
      architect: 'Takenaka Corporation + Nikken Sekkei',
      year: 2020,
      image_license: 'CC BY 4.0',
      image_credit: 'Photo: Nesnad / Wikimedia Commons',
    })

    const shiroiyaCase = payload.cases.find((item: { id: string }) => item.id === 'CASE-028')
    expect(shiroiyaCase).toMatchObject({
      name: '白井屋酒店',
      location: '前桥 日本',
    ...
    const acrosCase = payload.cases.find((item: { id: string }) => item.id === 'CASE-016')
    expect(acrosCase).toMatchObject({
      name: 'ACROS福冈',
      location: '福冈 日本',
      architect: 'Nihon Sekkei + Takenaka Corporation',
      year: 1995,
      image_license: 'CC BY-SA 2.0',
      image_credit: 'Kenta Mabuchi / Wikimedia Commons',
    })

    const oyuCase = payload.cases.find((item: { id: string }) => item.id === 'CASE-033')
    expect(oyuCase).toMatchObject({
      name: '大湯道路休息站',
      location: '鹿角 日本',
      architect: 'Kengo Kuma',
      year: 2018,
      image_license: 'CC BY-SA 4.0',
      image_credit: 'Photo: 掬茶 / Wikimedia Commons',
    })
  })
