...
import pack from '../../db/review-packets/graduation-library-batch-001.json'
import cases from '../../src/content/graduation/cases.json'

const root = process.cwd()
const applySql = fs.readFileSync(path.join(root, 'db/manual-operations/graduation-library-batch-001-apply.sql'), 'utf8')
const rollbackSql = fs.readFileSync(path.join(root, 'db/manual-operations/graduation-library-batch-001-rollback.sql'), 'utf8')

describe('graduation library batch 001 migration pack', () => {
  it('contains the complete reviewed batch with deterministic unique keys', () => {
    ...
    expect(new Set(pack.architects.map(item => item.id)).size).toBe(7)
    expect(new Set(pack.buildings.map(item => item.id)).size).toBe(8)
    expect(new Set(pack.buildings.map(item => item.slug)).size).toBe(8)
    expect(new Set(pack.images.map(item => item.id)).size).toBe(8)
    expect(new Set(pack.profiles.map(item => item.case_id)).size).toBe(8)
    expect(new Set(pack.assignments.map(item => `${item.building_id}:${item.function_slug}`)).size).toBe(17)
  })

  it('keeps canonical facts, image metadata and profile ownership synchronized', () => {
    ...
    for (const image of pack.images) {
      ...
      expect(building?.official_url).toBe(decision?.canonical_building.official_url)
  ...
  it('uses guarded transactional apply and dependency-aware rollback', () => {
    expect(applySql).toMatch(/^-- Graduation library batch 001/)
    ...
    expect(rollbackSql).toContain('Rollback refused: found % external relations')
